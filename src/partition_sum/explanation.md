...
Let `dp[i]` be the sum of the scores of all partitions of `arr[:i]`, and let `cnt[i]` be the number of ways to partition `arr[:i]`. Then, we have the formula:
    $$\text{dp}[i] = \underset{j = 0}{\overset{i - 1}{\sum}}(\text{dp}[j] + \text{cnt}[j]\cdot\text{mex}(\text{arr}[j:i])).$$
This is because the suffix `arr[j:i]` will be appended to all partitions of `arr[:j]` so that they together extend to index $i$, and so for each of the `cnt[j]` partitions making up `dp[j]`, it contributes `mex(arr[j:i])`. Note that we can rewrite the formula as $\text{dp}[i] = \underset{j = 0}{\overset{i - 1}{\sum}}\text{dp}[j] + \underset{j = 0}{\overset{i - 1}{\sum}}(\text{cnt}[j]\cdot\text{mex}(\text{arr}[j:i]))$, so `dp[i]` will by default contain the prefix sum $\underset{j = 0}{\overset{i - 1}{\sum}}\text{dp}[j]$. Therefore we can essentially ignore that part of the formula, and focus on the $\underset{j = 0}{\overset{i - 1}{\sum}}(\text{cnt}[j]\cdot\text{mex}(\text{arr}[j:i]))$ sum. 
...
At a given $i$, it once again makes sense to group suffixes with equal MEX together. If $\text{mex}(\text{arr[j:i]})$ is the same for all $j \in [j_l, j_r)$, then we can evaluate $\underset{j=j_l}{\overset{j_r - 1}{\sum}}\text{cnt}[j]\cdot\text{mex}(\text{arr}[j:i])$ in constant time using prefix counts. If $\text{pfx}[i] := \underset{j=0}{\overset{i - 1}{\sum}}\text{cnt}[j]$, then $\underset{j=j_l}{\overset{j_r - 1}{\sum}}\text{cnt}[j]\cdot\text{mex}(\text{arr}[j:i])=(\text{pfx}[j_r]-\text{pfx}[j_l])\cdot\text{mex}(\text{arr}[j_l:i])$.
